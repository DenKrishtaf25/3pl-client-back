...
@Injectable()
export class FinanceImportService implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    this.logger.log('Планировщик импорта finance инициализирован. Запуск первого импорта...')
    // Запускаем импорт асинхронно, чтобы не блокировать инициализацию модуля
    setImmediate(() => {
      this.handleFinanceImport()
    })
  }
  ...
  private scheduleNextImport() {
    ...
    this.logger.log(`Следующий импорт finance запланирован на ${nextRunTime.toLocaleString()}`)
  }

  async handleFinanceImport() {
    if (this.isRunning) {
      this.logger.warn('Импорт finance уже выполняется, пропускаем...')
      return
    }
    ...
    try {
      this.logger.log('Начинаем автоматический импорт finance...')
      
      // Определяем команду в зависимости от окружения
      const isProduction = process.env.NODE_ENV === 'production'
      const command = isProduction 
        ? 'npm run import:finance:prod'
      ...
      if (stderr) {
        this.logger.warn(`Импорт finance завершен с предупреждениями за ${duration} сек`)
        this.logger.debug(stderr)
      } else {
        this.logger.log(`Импорт finance успешно завершен за ${duration} сек`)
      }
      ...
      const duration = ((Date.now() - startTime) / 1000).toFixed(1)
      this.logger.error(`Ошибка при импорте finance за ${duration} сек:`, error)
  ...
  async importFinanceManually() {
    if (this.isRunning) {
      throw new Error('Импорт finance уже выполняется')
    }
    
    await this.handleFinanceImport()
  }
}
