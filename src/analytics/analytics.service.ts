...
export class AnalyticsService {
  ...
  async getChartData(dto: FindAnalyticsDto, userId: string, userRole: string) {
    ...
    if (userRole === 'ADMIN') {
      // Админ может видеть любые analytics
    ...
    const analyticsRecords = await this.prisma.analytics.findMany({
      where,
      orderBy: { date: 'asc' },
      select: {
        date: true,
        quantityByRequest: true,
        quantityByPlan: true,
        quantityByFact: true,
        quantityByDeparture: true,
      }
    })
    ...
    analyticsRecords.forEach(record => {
      ...
      if (existing) {
        existing.quantityByRequest += record.quantityByRequest
        existing.quantityByPlan += record.quantityByPlan
        existing.quantityByFact += record.quantityByFact
        existing.quantityByDeparture += record.quantityByDeparture
      } else {
        groupedData.set(dateKey, {
          date: record.date,
          quantityByRequest: record.quantityByRequest,
          quantityByPlan: record.quantityByPlan,
          quantityByFact: record.quantityByFact,
          quantityByDeparture: record.quantityByDeparture,
        })
      }
    })
    ...
    const chartData = Array.from(groupedData.values())
      ...
      .map(item => ({
        ...
        quantityByRequest: item.quantityByRequest,
        quantityByPlan: item.quantityByPlan,
        quantityByFact: item.quantityByFact,
        quantityByDeparture: item.quantityByDeparture,
      }))
